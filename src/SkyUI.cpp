...
auto Config::GetBool(ScriptObjectPtr a_object, std::string_view a_variableName) -> bool
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	return variable ? variable->GetBool() : false;
}

void Config::SetBool(ScriptObjectPtr a_object, std::string_view a_variableName, bool a_value)
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	if (variable)
...
auto Config::GetInt(ScriptObjectPtr a_object, std::string_view a_variableName) -> std::int32_t
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	return variable ? variable->GetSInt() : 0;
}

void Config::SetInt(ScriptObjectPtr a_object, std::string_view a_variableName, std::int32_t a_value)
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	if (variable)
...
auto Config::GetFloat(ScriptObjectPtr a_object, std::string_view a_variableName) -> float
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	return variable ? variable->GetFloat() : 0.0f;
}

void Config::SetFloat(ScriptObjectPtr a_object, std::string_view a_variableName, float a_value)
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	if (variable)
...
auto Config::GetString(
	ScriptObjectPtr a_object,
	std::string_view a_variableName) -> std::string
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	return variable ? std::string{ variable->GetString() } : ""s;
}

void Config::SetString(
	ScriptObjectPtr a_object,
...
auto Config::GetArray(ScriptObjectPtr a_object, std::string_view a_variableName) -> ScriptArrayPtr
{
	auto variable = Utils::GetVariable(a_object, a_variableName);
	return variable ? variable->GetArray() : nullptr;
}

void Config::RegisterForModEvent(
	ScriptObjectPtr a_object,
...
void Config::UnregisterForModEvent(
	ScriptObjectPtr a_object,
	RE::BSFixedString a_eventName)
{
	auto skyrimVM = RE::SkyrimVM::GetSingleton();
	...
	bool a_noUpdate)
{
	if (GetInt(a_object, "_state"sv) == static_cast<std::int32_t>(State::Reset))
	{
		Error(
			a_object,
			"Cannot set option flags while in OnPageReset(). Pass flags to AddOption instead"sv);
		return;
	}
